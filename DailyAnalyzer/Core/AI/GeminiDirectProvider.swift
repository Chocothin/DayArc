...
final class GeminiDirectProvider: LLMProvider {
    ...
    private var languageInstruction: String {
        let langRaw = UserDefaults.standard.string(forKey: "reportLanguage") ?? "ko"
        let isKorean = langRaw == "ko"
        
        if isKorean {
            return """
            You MUST generate the 'title', 'summary', and 'detailedSummary' fields in KOREAN (한국어).
            However, keep 'category', 'subcategory', and 'appSites' in English as they are system identifiers.
            """
        } else {
            return """
            You MUST generate the 'title', 'summary', and 'detailedSummary' fields in ENGLISH.
            """
        }
    }
    ...
    private func logRequestTiming(context: String) {
        ...
        Self.requestQueue.sync {
            ...
            if let last = Self.lastRequestTime {
                ...
                print("GEMINI TIMING: \(context) - \(String(format: "%.1f", interval))s since last request")
            } else {
                print("GEMINI TIMING: \(context) - First request")
            }
    ...
    func transcribeVideo(videoData: Data, mimeType: String, prompt: String, batchStartTime: Date, videoDuration: TimeInterval, batchId: Int64?) async throws -> (observations: [Observation], log: LLMCall) {
        ...
        let basePrompt = PromptManager.shared.getPrompt(for: .videoTranscription)
            ...
            do {
                ...
                if let nsError = error as NSError?,
                   ...
                   let transition = modelState.advance() {
                    ...
                    #if DEBUG
                    print("Downgrading to \(transition.to.rawValue) after \(nsError.code)")
                    #endif
    ...
    func generateActivityCards(observations: [Observation], context: ActivityGenerationContext, batchId: Int64?) async throws -> (cards: [ActivityCardData], log: LLMCall) {
        ...
        let detailedPrompt = PromptManager.shared.getPrompt(for: .activityCardDetailed)
        ...
        while attempt < maxRetries {
            do {
                ...
                #if DEBUG
                print("Validation failed on attempt \(attempt + 1)")
                #endif
        ...
        for cycle in 1...maxCycles {
            ...
            while Date().timeIntervalSince(startTime) < maxWaitTime {
                do {
                    ...
                    #if DEBUG
                    print("Error checking file status: \(error.localizedDescription)")
                    #endif
                    lastError = error
                }
        ...
        do {
            ...
            if httpResponse.statusCode >= 400 {
                if httpResponse.statusCode == 503, let recovered = recover503CandidateText(data) {
                    #if DEBUG
                    print("HTTP 503 received, but valid candidate payload was recovered; treating as success.")
                    #endif
                    ...
                    print("HTTP 503 contained no recoverable payload. preview=\(preview)")
                    #endif
