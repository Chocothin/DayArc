...
final class OllamaProvider: LLMProvider {
    ...
    private func getSimpleFrameDescription(_ frame: FrameData, batchId: Int64?) async -> String? {
        ...
        guard let base64String = String(data: frame.image, encoding: .utf8) else {
            #if DEBUG
            print("[OLLAMA] WARNING: Failed to decode frame image - skipping frame")
            #endif
            return nil
        }
        ...
        do {
            ...
            #if DEBUG
            print("[OLLAMA] WARNING: describe_frame failed at \(frame.timestamp)s - skipping frame: \(error.localizedDescription)")
            #endif
            return nil
        }
    }
    ...
    private func generateSummary(observations: [Observation], categories: [LLMCategoryDescriptor], batchId: Int64?) async throws -> (SummaryResponse, String) {
        ...
        let summaryPrompt = PromptManager.shared.getPrompt(for: .activityCardSummary)
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: generateSummary attempt \(attempt) failed: \(error.localizedDescription)")
                #endif
    ...
    private func generateTitle(summary: String, batchId: Int64?) async throws -> (TitleResponse, String) {
        ...
        // Get prompt from PromptManager
        let titlePrompt = PromptManager.shared.getPrompt(for: .activityCardTitle)
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: generateTitle attempt \(attempt) failed: \(error.localizedDescription)")
                #endif
    ...
    private func checkShouldMerge(previousCard: ActivityCardData, newCard: ActivityCardData, batchId: Int64?) async throws -> (Bool, String) {
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: evaluate_card_merge attempt \(attempt) failed: \(error.localizedDescription)")
                #endif
    ...
    private func mergeTwoCards(previousCard: ActivityCardData, newCard: ActivityCardData, batchId: Int64?) async throws -> (ActivityCardData, String) {
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: merge_cards attempt \(attempt) failed: \(error.localizedDescription)")
                #endif
    ...
    private func convertSegmentsToObservations(_ segments: [VideoSegment],
        ...
        for (index, segment) in segments.enumerated() {
            ...
            if startSeconds < -tolerance || endSeconds > videoDuration + tolerance {
                #if DEBUG
                print("[OLLAMA] WARNING: Segment \(index + 1) exceeds video duration: \(segment.startTimestamp)-\(segment.endTimestamp) (video is \(durationString))")
                #endif
                continue
            }
            ...
            if let prevEnd = lastEndTime {
                ...
                if gap > 60.0 {
                    #if DEBUG
                    print("[OLLAMA] WARNING: Gap of \(Int(gap))s between segments at \(String(format: "%02d:%02d", Int(prevEnd) / 60, Int(prevEnd) % 60))")
                    #endif
                }
            }
        ...
        #if DEBUG
        if coverage > 1.2 {
            print("[OLLAMA] WARNING: Segments exceed video duration by \(Int((coverage - 1) * 100))%")
        }
        #endif
    ...
    private func mergeFrameDescriptions(_ frameDescriptions: [(timestamp: TimeInterval, description: String)],
        ...
        for attempt in 1...maxAttempts {
            do {
                ...
                if attempt == maxAttempts {
                    #if DEBUG
                    print("[OLLAMA] WARNING: segment_video_activity retries exhausted (coverage) - returning raw frame observations")
                    #endif
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: Segment coverage attempt \(attempt) only reached \(coveragePercent)% - retrying")
                #endif
                ...
                if attempt == maxAttempts {
                    #if DEBUG
                    print("[OLLAMA] WARNING: segment_video_activity retries exhausted (error: \(error.localizedDescription)) - returning raw frame observations")
                    #endif
                    return try observationsFromFrames(
                        frameDescriptions,
                        batchStartTime: batchStartTime,
                ...
                #if DEBUG
                print("[OLLAMA] WARNING: segment_video_activity attempt \(attempt) failed: \(error.localizedDescription)")
                #endif
