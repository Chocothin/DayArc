...
final class ScreenRecorder: NSObject, SCStreamOutput {
    ...
    init(autoStart: Bool = true) {
        ...
        if autoStart, AppState.shared.isRecordingEnabled { start() }
    ...
    private func forceStopFlag(reason: String, userInitiated: Bool, extra: [String: Any] = [:]) {
        AppState.shared.isRecordingEnabled = false
    ...
    private func finishSegment(restart: Bool = true) {
        ...
        w.finishWriting { [weak self] in
            ...
            self.q.async { [weak self] in
                ...
                Task { @MainActor in
                    guard AppState.shared.isRecordingEnabled else { return }
                    self.q.async { [weak self] in
                        guard let self else { return }
    ...
    private func registerForSleepAndLock() {
        ...
        nc.addObserver(forName: NSWorkspace.willSleepNotification,
            ...
            self.q.async { [weak self] in
                ...
                Task { @MainActor in
                    if AppState.shared.isRecordingEnabled {
                        self.q.async { [weak self] in
    ...
    private func resumeRecording(after delay: TimeInterval, context: String) {
        q.asyncAfter(deadline: .now() + delay) { [weak self] in
            guard let self else { return }
            Task { @MainActor in
                guard AppState.shared.isRecordingEnabled else {
