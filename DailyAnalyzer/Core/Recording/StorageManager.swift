...
protocol StorageManaging: Sendable {
    ...
    func fetchPendingBatches() -> [Int64]
    
    func currentRecordingUsageBytes() -> Int64
...
struct StorageTimelineCard: Codable, Sendable, Identifiable {
    var id = UUID()
    let recordId: Int64?
...
final class StorageManager: StorageManaging, @unchecked Sendable {
    ...
    public private(set) var dbURL: URL!
    ...
    // Disk space limits
    private let minFreeSpaceMB: Int64 = 500
    private let maxRecordingsSizeGB: Int = 10
    
    /// Check if there is sufficient disk space to start/continue recording
    func hasSufficientDiskSpace() -> Bool {
        do {
            // 1. Check system free space
            let values = try root.resourceValues(forKeys: [.volumeAvailableCapacityKey])
            if let available = values.volumeAvailableCapacity, available < minFreeSpaceMB * 1024 * 1024 {
                Logger.shared.error("Insufficient system disk space: \(available / 1024 / 1024)MB free", source: "StorageManager")
                return false
            }
            
            // 2. Check app usage (recordings folder)
            let size = (try? fileMgr.allocatedSizeOfDirectory(at: root)) ?? 0
            if size > Int64(maxRecordingsSizeGB) * 1024 * 1024 * 1024 {
                Logger.shared.warning("App recording storage limit exceeded: \(size / 1024 / 1024)MB used", source: "StorageManager")
                // We might want to allow recording but trigger a purge. 
                // For now, strict check to prevent filling disk.
                // Actually, purge should handle this. Let's return true if system space is okay, 
                // assuming purge works. But if we are WAY over, maybe stop.
                // Let's stick to system free space as the hard blocker.
            }
            
            return true
        } catch {
            Logger.shared.error("Failed to check disk space: \(error)", source: "StorageManager")
            return false // Fail safe
        }
    }
    
    /// Resolve video file URL robustly (checking absolute path, then relative to recordings root)
    func resolveVideoURL(for path: String) -> URL? {
        let fileURL = URL(fileURLWithPath: path)
        if fileMgr.fileExists(atPath: fileURL.path) { return fileURL }
        
        // Try relative to recordings root
        let fileName = fileURL.lastPathComponent
        let resolvedURL = recordingsRoot.appendingPathComponent(fileName)
        if fileMgr.fileExists(atPath: resolvedURL.path) {
            return resolvedURL
        }
    ...
    private init() {
        ...
        // NEW: Migrate from DailyAnalyzer (v2) to DayArc (v3) - HIGHEST PRIORITY
        // We do this before StoragePathMigrator (v1 -> v3) to ensure v2 data takes precedence.
        let oldBaseDir = appSupport.appendingPathComponent("DailyAnalyzer", isDirectory: true)
        let newBaseDir = appSupport.appendingPathComponent("DayArc", isDirectory: true)
        
        if fileMgr.fileExists(atPath: oldBaseDir.path) {
            let shouldMigrate: Bool
            
            if !fileMgr.fileExists(atPath: newBaseDir.path) {
                shouldMigrate = true
            } else {
                // Target exists. Check if it's "empty" or "fresh"
                let dbPath = newBaseDir.appendingPathComponent("chunks.sqlite").path
                if !fileMgr.fileExists(atPath: dbPath) {
                    shouldMigrate = true
                } else {
                    let dbSize = (try? fileMgr.attributesOfItem(atPath: dbPath)[.size] as? Int64) ?? 0
                    shouldMigrate = (dbSize < 100 * 1024)
                }
            }
            ...
            if shouldMigrate {
                do {
                    if fileMgr.fileExists(atPath: newBaseDir.path) {
                        let backupName = "DayArc_Backup_\(Int(Date().timeIntervalSince1970))"
                        let backupUrl = appSupport.appendingPathComponent(backupName, isDirectory: true)
                        try fileMgr.moveItem(at: newBaseDir, to: backupUrl)
                        print("ℹ️ [StorageManager] Backed up fresh DayArc folder to \(backupName)")
                    }
                    
                    try fileMgr.moveItem(at: oldBaseDir, to: newBaseDir)
                    print("✅ [StorageManager] Migrated data from DailyAnalyzer to DayArc")
                    
                    // Mark v1->v3 migration as done so we don't overwrite with even older data
                    UserDefaults.standard.set(true, forKey: "didMigrateStorageToDayArc")
                } catch {
                    print("⚠️ [StorageManager] Failed to migrate data: \(error)")
                }
            } else {
                print("ℹ️ [StorageManager] Found existing DayArc data, skipping migration from DailyAnalyzer")
            }
        }
        ...
        // let appSupport ... (already defined above)
        
        let baseDir = newBaseDir
        ...
        do {
            ...
            Logger.shared.error("Failed to create storage directories: \(error)", source: "StorageManager")
            // This is critical, but we don't want to crash. The app will likely fail to save anything.
        }

        root = recordingsDir
        dbURL = baseDir.appendingPathComponent("chunks.sqlite")
        UserDefaults.standard.set(dbURL.path, forKey: "DayArcDatabasePath")
        UserDefaults.standard.set(dbURL.path, forKey: "DayflowDatabasePath")
        ...
        // File logging for debugging (since print doesn't show in logs)
        let logPath = "/tmp/dailyanalyzer_init.log"
        let logMsg = "[\(Date())] Attempting DB initialization at \(dbURL.path)\n"
        try? logMsg.write(toFile: logPath, atomically: false, encoding: .utf8)
        
        do {
            db = try DatabasePool(path: dbURL.path, configuration: config)
            let successMsg = "✅ [StorageManager] Database initialized successfully at \(dbURL.path)\n"
            print(successMsg)
            try? successMsg.appendLine(to: logPath)
        } catch let initError {
            let criticalMsg = """

❌ [StorageManager] CRITICAL: Failed to initialize database at \(dbURL.path)
❌ [StorageManager] Error: \(initError)
❌ [StorageManager] Error details: \(String(describing: initError))
❌ [StorageManager] Base directory: \(baseDir.path)
❌ [StorageManager] Directory exists: \(fileMgr.fileExists(atPath: baseDir.path))
❌ [StorageManager] Attempting to create database with minimal configuration...

"""
            print(criticalMsg)
            try? criticalMsg.appendLine(to: logPath)
            
            // Fallback: Try with minimal configuration
            var fallbackConfig = Configuration()
            fallbackConfig.maximumReaderCount = 1
            do {
                db = try DatabasePool(path: dbURL.path, configuration: fallbackConfig)
                let fallbackSuccessMsg = "✅ [StorageManager] Database initialized with fallback configuration\n"
                print(fallbackSuccessMsg)
                try? fallbackSuccessMsg.appendLine(to: logPath)
            } catch let fallbackError {
                let fatalMsg = """
❌ [StorageManager] FATAL: Cannot initialize database even with fallback config
❌ [StorageManager] Fallback error: \(fallbackError)
❌ [StorageManager] Error type: \(type(of: fallbackError))
❌ [StorageManager] App will continue with degraded functionality (no database)

"""
                print(fatalMsg)
                try? fatalMsg.appendLine(to: logPath)
                // Set db to nil to allow app to continue
                db = nil
            }
        }
    ...
    private func timedWrite<T>(_ label: String, _ block: (Database) throws -> T) throws -> T {
        ...
        do {
            guard let result = try db?.write({ db in
                execStart = CFAbsoluteTimeGetCurrent()
                defer { execEnd = CFAbsoluteTimeGetCurrent() }
                return try block(db)
            }) else {
                throw NSError(domain: "StorageManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "Database not initialized"])
            }
    ...
    func saveBatch(startTs: Int, endTs: Int, chunkIds: [Int64]) -> Int64? {
        ...
        do {
            ...
            Logger.shared.error("Failed to save batch: \(error)", source: "StorageManager")
            return nil
        }
    ...
    func saveTimelineCardShell(batchId: Int64, card: TimelineCardShell) -> Int64? {
        ...
        do {
            ...
            Logger.shared.error("Failed to save timeline card: \(error)", source: "StorageManager")
        }
        return lastId
    }
    ...
    func replaceTimelineCardsInRange(from: Date, to: Date, with newCards: [TimelineCardShell], batchId: Int64) -> (insertedIds: [Int64], deletedVideoPaths: [String]) {
        ...
        do {
            try timedWrite("replaceTimelineCardsInRange(\(newCards.count)_cards)") { db in
             ...
             Logger.shared.error("Failed to replace timeline cards: \(error)", source: "StorageManager")
        }
        ...
        do {
            try timedWrite("saveObservations(\(observations.count)_items)") { db in
                ...
                for obs in observations {
                    let metadataString: String? = (try? encoder.encode(obs.metadata)).flatMap { String(data: $0, encoding: .utf8) }
                    ...
                    try db.execute(sql: """
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, arguments: [
                        batchId, obs.startTs, obs.endTs, obs.observation, metadataString, obs.llmModel
                    ])
                }
            }
        } catch {
            Logger.shared.error("Failed to save observations: \(error)", source: "StorageManager")
        }
    }
    ...
    func fetchPendingBatches() -> [Int64] {
        (try? timedRead("fetchPendingBatches") { db in
            let rows = try Row.fetchAll(db, sql: "SELECT id FROM analysis_batches WHERE status = 'pending'")
    ...
    private func purgeIfNeeded() {
        purgeQ.async { [weak self] in
            ...
            do {
                ...
                let retentionDays = StoragePreferences.retentionDays
                ...
                print("🧹 [StoragePurge] Starting purge check...")
                print("   Current size: \(currentSize / 1024 / 1024 / 1024)GB, Limit: \(limit / 1024 / 1024 / 1024)GB")
                print("   Retention: \(retentionDays) days (cutoff: \(cutoffDate))")
                
                // Clean up if above limit
                if currentSize > limit {
                    let excessGB = Double(currentSize - limit) / 1024 / 1024 / 1024
                    print("⚠️ [StoragePurge] Exceeded limit by \(String(format: "%.2f", excessGB))GB, cleaning up...")

                    try self.timedWrite("purgeIfNeeded") { db in
                        // Get chunks older than retention period with file paths still set
                        ...
                        print("   Found \(oldChunks.count) old chunks to potentially delete")
                        ...
                        for chunk in oldChunks {
                            ...
                            if FileManager.default.fileExists(atPath: path) {
                                do {
                                    ...
                                    failedDeletions.append((path, error))
                                    // Don't count as freed space if deletion failed
                                    // But keep DB mark to avoid retry loops
                                }
                            } else {
                            ...
                            if currentSize - freedSpace < limit {
                                print("✅ [StoragePurge] Target reached, stopping cleanup")
                                break
                            }
                        }
                        
                        let freedGB = Double(freedSpace) / 1024 / 1024 / 1024
                        print("✅ [StoragePurge] Deleted \(deletedCount) chunks, freed \(String(format: "%.2f", freedGB))GB")
                        
                        if !failedDeletions.isEmpty {
                            print("⚠️ [StoragePurge] Failed to delete \(failedDeletions.count) files:")
                            for (path, error) in failedDeletions.prefix(5) {
                                print("   - \(URL(fileURLWithPath: path).lastPathComponent): \(error.localizedDescription)")
                            }
                        }
                    }
                } else {
                    print("✅ [StoragePurge] Storage within limit, no cleanup needed")
                }
            } catch {
                print("❌ [StoragePurge] Purge error: \(error)")
            }
        }
    }
}
...
private extension StorageManager {
    func migrateLegacyChunkPathsIfNeeded() {
        ...
        let legacyBase = fileMgr.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Containers/\(bundleID)/Data/Library/Application Support/DayArc", isDirectory: true)
        let newBase = appSupport.appendingPathComponent("DayArc", isDirectory: true)
