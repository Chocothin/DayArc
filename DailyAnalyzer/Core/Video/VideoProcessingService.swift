...
actor VideoProcessingService {
    ...
    // 🔧 FIX: Shared queue for timelapse processing to prevent thread accumulation
    private let timelapseProcessingQueue = DispatchQueue(
        label: "com.dayarc.timelapse.processing",
        qos: .userInitiated,
        attributes: [],
    ...
    init() {
        ...
        self.persistentTimelapsesRootURL = appSupportURL.appendingPathComponent("DayArc/timelapses", isDirectory: true)
    ...
    func generatePersistentTimelapseURL(for date: Date,
        ...
        do {
            try fileManager.createDirectory(at: dateSpecificDir,
                                            withIntermediateDirectories: true,
                                            attributes: nil)
            print("✅ Created timelapse directory: \(dateSpecificDir.path)")
        } catch {
    ...
    func generateTimelapse(sourceVideoURL: URL,
                          ...
                          speedupFactor: Int = 8,
                          outputFPS: Int = 15) async throws {
        print("⏳ [TIMELAPSE] Starting generation:")
        print("   Source: \(sourceVideoURL.lastPathComponent)")
        print("   Output: \(outputTimelapseFileURL.path)")
        print("   Speedup: \(speedupFactor)x, FPS: \(outputFPS)")
        
        let asset = AVURLAsset(url: sourceVideoURL)
        guard let assetTrack = try await asset.loadTracks(withMediaType: .video).first else {
            print("❌ [TIMELAPSE] Error: No video track in \(sourceVideoURL.lastPathComponent)")
            throw VideoProcessingError.noVideoTracks
        }
        ...
        guard let compositionTrack = composition.addMutableTrack(withMediaType: .video,
                                                                preferredTrackID: kCMPersistentTrackID_Invalid) else {
            print("❌ [TIMELAPSE] Failed to create composition track")
            throw VideoProcessingError.noVideoTracks
        }
        ...
        if !fileManager.fileExists(atPath: outputDir.path) {
            do {
                try fileManager.createDirectory(at: outputDir,
                                               withIntermediateDirectories: true,
                                               attributes: nil)
                print("✅ [TIMELAPSE] Created output directory: \(outputDir.path)")
            } catch {
                print("❌ [TIMELAPSE] Failed to create directory: \(error)")
                throw VideoProcessingError.directoryCreationFailed(error)
            }
        }
        ...
        if fileManager.fileExists(atPath: outputTimelapseFileURL.path) {
            do {
                ...
                print("🗑️  [TIMELAPSE] Removed existing file")
            } catch {
                print("⚠️  [TIMELAPSE] Warning: Could not remove existing file: \(error)")
            }
        }
        ...
        guard let writer = try? AVAssetWriter(outputURL: outputTimelapseFileURL, fileType: .mp4) else {
            print("❌ [TIMELAPSE] Failed to create AVAssetWriter")
        ...
        guard writer.canAdd(writerInput) else {
            print("❌ [TIMELAPSE] Cannot add writer input")
        ...
        guard let reader = try? AVAssetReader(asset: composition) else {
            print("❌ [TIMELAPSE] Failed to create AVAssetReader")
            throw VideoProcessingError.assetReaderCreationFailed(nil)
        }
        
        // 🔧 FIX: Track resources for cleanup in defer block
        // These will be set to nil after successful completion, but cleaned up on error
        var readerToCleanup: AVAssetReader? = reader
        var writerToCleanup: AVAssetWriter? = writer
        
        // 🔧 CRITICAL: Ensure resources are always cleaned up, even on error
        defer {
            if let r = readerToCleanup {
                r.cancelReading()
            }
            if let w = writerToCleanup, w.status == .writing {
                w.cancelWriting()
            }
            print("🧹 [TIMELAPSE] Resources cleaned up (reader: \(readerToCleanup != nil), writer: \(writerToCleanup != nil))")
        }
        ...
        guard reader.canAdd(readerOutput) else {
            print("❌ [TIMELAPSE] Cannot add reader output")
            throw VideoProcessingError.assetReaderCreationFailed(nil)
        }
        ...
        guard writer.startWriting() else {
            print("❌ [TIMELAPSE] Writer failed to start: \(writer.error?.localizedDescription ?? "unknown error")")
            throw VideoProcessingError.assetWriterStartFailed(writer.error)
        }
        writer.startSession(atSourceTime: .zero)
        reader.startReading()
        
        print("⚙️  [TIMELAPSE] Processing frames with shared queue...")
        
        
        // Process video frames
        await withCheckedContinuation { continuation in
            writerInput.requestMediaDataWhenReady(on: timelapseProcessingQueue) {
                    ...
                    if reader.status == .reading,
                       let sampleBuffer = readerOutput.copyNextSampleBuffer() {
                        ...
                        writer.finishWriting {
                            // Clear tracking references before resuming
                            readerToCleanup = nil
                            writerToCleanup = nil
                            continuation.resume()
                        }
