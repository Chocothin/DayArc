...
final class GeminiDirectProvider: LLMProvider {
    ...
    private func normalizeCategory(_ raw: String, descriptors: [LLMCategoryDescriptor]) -> String {
        ...
        // 2. Idle category special handling
        if let idle = descriptors.first(where: { $0.isIdle }) {
        ...
        // 3. Partial match - check if LLM output contains category name
        if let match = descriptors.first(where: { normalized.contains($0.name.lowercased()) }) {
            return match.name
        }

        // 4. Common aliases for better categorization
        let personalKeywords = ["personal", "private", "home", "shopping", "career development", "job hunting", "interview", "이직", "면접", "쇼핑", "개인"]
        let distractionKeywords = ["distraction", "break", "entertainment", "leisure", "browsing", "youtube", "social media", "news"]
        let workKeywords = ["work", "job", "office", "meeting", "project", "업무", "회의"]

        if personalKeywords.contains(where: { normalized.contains($0) }) {
            if let personal = descriptors.first(where: { $0.name.lowercased() == "personal" }) {
                return personal.name
            }
        }
        if distractionKeywords.contains(where: { normalized.contains($0) }) {
            if let distraction = descriptors.first(where: { $0.name.lowercased() == "distraction" }) {
                return distraction.name
            }
        }
        if workKeywords.contains(where: { normalized.contains($0) }) {
            if let work = descriptors.first(where: { $0.name.lowercased() == "work" }) {
                return work.name
            }
        }

        // 5. If no match, return the cleaned value as-is (don't default to Work)
        // This preserves the LLM's output for debugging and allows manual review
        #if DEBUG
        print("⚠️ Category normalization: '\(cleaned)' did not match any category, keeping as-is")
        #endif
        return cleaned
    }
    ...
    func generateActivityCards(observations: [Observation], context: ActivityGenerationContext, batchId: Int64?) async throws -> (cards: [ActivityCardData], log: LLMCall) {
        ...
        - IMPORTANT: Look at the macOS menu bar app name, window title, and distinctive UI elements to identify the correct app.
        ...
        Native macOS Apps (use these exact identifiers):
        - VS Code → code.visualstudio.com (blue icon, "Visual Studio Code" in menu bar)
        - Cursor → cursor.com (purple/blue gradient icon, "Cursor" in menu bar)
        - Xcode → developer.apple.com/xcode (blue hammer icon, "Xcode" in menu bar)
        - Obsidian → obsidian.md (purple gem icon, "Obsidian" in menu bar, vault name in title)
        - Sublime Text → sublimetext.com (orange/yellow icon)
        - IntelliJ IDEA → jetbrains.com/idea (red/pink square icon, "IntelliJ IDEA" in menu bar)
        - PyCharm → jetbrains.com/pycharm
        - WebStorm → jetbrains.com/webstorm
        - Terminal → apple.com/terminal
        - iTerm2 → iterm2.com
        - Warp → warp.dev
        - Arc Browser → arc.net
        ...
        - Notes (Apple) → apple.com/notes
        - Bear → bear.app
        - Craft → craft.do
        - Logseq → logseq.com

        COMMONLY CONFUSED APPS - Pay close attention:
        - Obsidian vs VS Code: Obsidian has purple gem logo, shows "Obsidian" in menu bar, has vault name in window title, uses [[wikilinks]] syntax. VS Code shows "Visual Studio Code" or "Code" in menu bar, has blue icon, shows file paths in title.
        - Notion vs Obsidian: Notion is web-based (usually in browser), has black/white logo, "Notion" in tab. Obsidian is native app with purple icon.
        - Cursor vs VS Code: Cursor has purple/gradient icon, shows "Cursor" in menu bar. VS Code has solid blue icon.
        - iTerm2 vs Terminal: iTerm2 has green icon and more customization. Terminal has black/white icon.
        - JetBrains IDEs: Each has distinct colored icon - IntelliJ (red), PyCharm (green/yellow), WebStorm (blue/cyan). Check menu bar for exact product name.
