...
struct ScreenRecordingPermissionView: View {
    ...
    private func quitAndReopen() {
        Task { @MainActor in
            ...
            // Get the exact path of the currently running app
            let appPath = Bundle.main.bundlePath

            // Use a shell script to relaunch this exact app after a short delay
            // -n flag forces opening this specific instance, bypassing Launch Services cache
            // This ensures the same app instance (e.g., DerivedData build) is reopened
            let script = """
            sleep 1
            open -n "\(appPath)"
            """
