...
## 포함된 것
- `DayflowEngine.swift`: 퍼사드 + 설정 구조체. `allowWrites` 기본값 `false`.
- 스캐폴드/스탑게이트: `Types.swift`, `StorageScaffold.swift`, `RecordingScaffold.swift`, `LLMScaffold.swift`, `NoOpTelemetry.swift`.
- 스테이징된 Dayflow 원본 소스(빌드 미포함): `Source/` 이하 Recording/Storage/Analysis/AI/Models/Utilities/Security.
- 문서: `ModuleNotes.md`, `IntegrationPlan.md`, `DependencyNotes.md`.
